...
try:
    ...
    from .MAB import MAB, MarkovianMAB, ChangingAtEachRepMAB, NonStationaryMAB, PieceWiseStationaryMAB, IncreasingMAB, Op_MAB
...
class Evaluator(object):
    ...
    def __init__(self, configuration,
        ...
        self.MaxRewards = np.zeros((self.nbPolicies, len(self.envs), self.horizon))
        ...
        if STORE_MAX_REWARDS:
            self.allMaxRewards = np.zeros((self.nbPolicies, len(self.envs), self.horizon, self.repetitions))
    ...
    def __initEnvironments__(self):
        ...
        for configuration_arms in self.cfg['environment']:
            ...
            if new_mab_problem is None:
                ...
                new_mab_problem = Op_MAB(configuration_arms)
    ...
    def startOneEnv(self, envId, env):
        ...
        def store(r, policyId, repeatId):
            ...
            max_rewards = np.zeros(r.rewards.shape)
            cur_reward = -np.inf
            for ind, reward in enumerate(r.rewards):
                if reward > cur_reward:
                    cur_reward = reward
                max_rewards[ind] = cur_reward
            ...
            if hasattr(self, 'MaxRewards'):
                self.MaxRewards[policyId, envId, :] += max_rewards
            if hasattr(self, 'allMaxRewards'):
                self.allMaxRewards[policyId, envId, :, repeatId] = max_rewards
        ...
        for policyId, policy in enumerate(self.policies):
            ...
            self.store_dic[f'{self.policies[policyId].__cachedstr__}'] = {}
            self.store_dic[f'{self.policies[policyId].__cachedstr__}']['rewards'] = self.rewards[policyId, envId, :]
            self.store_dic[f'{self.policies[policyId].__cachedstr__}']['maxrewards'] = self.MaxRewards[policyId, envId, :]
            self.store_dic[f'{self.policies[policyId].__cachedstr__}']['allmaxrewards'] = self.allMaxRewards[policyId, envId]
        with open(json_file, 'w') as f:
            json.dump(self.store_dic, f, cls=NumpyEncoder)
            # for repeatId in tqdm(range(self.repetitions), desc="Repeat"):
            #     r = delayed_play(env, policy, self.horizon, random_shuffle=self.random_shuffle, random_invert=self.random_invert, nb_break_points=self.nb_break_points, allrewards=allrewards, repeatId=repeatId, useJoblib=self.useJoblib)
            #     store(r, policyId, repeatId)
    ...
    def getMaxRewards(self, policyId, envId=0):
        return self.MaxRewards[policyId, envId, :] / float(self.repetitions)
    ...
    def plotRegrets(self, envId=0,
                    savefig=None, meanReward=False, maxReward=False,
        ...
        for i, policy in enumerate(self.policies):
            ...
            if semilogx or loglog:
                ...
                if maxReward:
                    plot_method(X[::self.delta_t_plot], Y[::self.delta_t_plot], label=f'{policy.__cachedstr__}({Y[-1]:.2f})', color=colors[i], marker=markers[i], markevery=(i / 50., 0.1), lw=lw, ms=int(1.5*lw))
                else:
                    plot_method(X[::self.delta_t_plot], Y[::self.delta_t_plot], label=f'{policy.__cachedstr__}', color=colors[i], marker=markers[i], markevery=(i / 50., 0.1), lw=lw, ms=int(1.5*lw))
        ...
        if meanReward:
            ...
            legend()
            plt.ylabel(r"Max reward, average on time $\tilde{r}_t = \frac{1}{t} \sum_{s=1}^{t}$ %s%s" % (r"$\sum_{k=1}^{%d} \mu_k\mathbb{E}_{%d}[T_k(t)]$" % (self.envs[envId].nbArms, self.repetitions) if moreAccurate else r"$\mathbb{E}_{%d}[r_s]$" % (self.repetitions), ylabel2))
            plt.title("Max rewards for different bandit algorithms, averaged ${}$ times\n${}$ arms{}: {}".format(self.repetitions, self.envs[envId].nbArms, self.envs[envId].str_sparsity(), self.envs[envId].reprarms(1, latex=True)))
    ...
    for t in prettyRange:
        ...
        if allrewards is None:
            if type(choice) is tuple:
                reward = env.draw(choice[0], size=choice[1])
                pass_list += list(range(t, t+choice[1]))
            else:
                reward = env.draw(choice)
        else:
            reward = allrewards[choice, repeatId, t]
        # 3. The policy sees the reward
        if type(choice) is tuple:
            policy.getReward(choice[0], reward)
        else:
            policy.getReward(choice, reward)

        # 4. Finally we store the results
        if type(choice) is tuple:
            result.store(t, choice[0], reward)
        else:
