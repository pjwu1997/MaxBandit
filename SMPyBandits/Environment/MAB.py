...
# --- Optimization MAB (Non-stationary MAB)
class Op_MAB(object):
    """ Basic Multi-Armed Bandit problem, for arms with optimization.
    ...
    def __init__(self, configuration):
        ...
        # Not usable for Non-stationaty Bandits
    ...
    def draw(self, armId, t=1, size=1):
        """ Return a random sample from the armId-th arm, at time t. Usually t is not used."""
        if size > 1:
            return self.arms[armId].draw_nparray(size)
        else:
            return self.arms[armId].draw(1)

    # def draw_nparray(self, armId, shape=(1,)):
    #     """ Return a numpy array of random sample from the armId-th arm, of a certain shape."""
    #     return self.arms[armId].draw_nparray(shape)

    def draw_nparray(self, armId, size):
        """ Return a numpy array of random sample from the armId-th arm, of a certain shape."""
        return self.arms[armId].draw_nparray(shape)
